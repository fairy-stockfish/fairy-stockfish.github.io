...
### What is NNUE?
NNUE are efficiently updateable neural networks, which were first applied to shogi, then later ported to official Stockfish. From there they made their way into Fairy-Stockfish. Several generalizations have been applied to the architecture of the neural network so that it can be used for different board sizes, arbitrary piece types, variants without kings, as well as variants with piece drops. See the [wiki of the NNUE training code](https://github.com/ianfab/variant-nnue-pytorch/wiki/Technical-details#variant-nnue-halfkav2-architecture) for technical details on these generalizations.

An overview over NNUE in general (not specific to variants or Fairy-Stockfish) can be found in the [chessprogramming wiki](https://www.chessprogramming.org/Stockfish_NNUE) as well as in the [documentation of the training code](https://github.com/ianfab/variant-nnue-pytorch/blob/master/docs/nnue.md).
...
| checkmateless | [checkmateless-b6c205ca535b.nnue](https://drive.google.com/u/0/uc?id=1LSNQVAiriIpTXCAOKzxhtC_hmKz17S5h&export=download) | +333 | 2022/04/18 | Belzedar#8832 | [Custom defined variant](https://github.com/ianfab/Fairy-Stockfish/wiki/Variant-configuration#checkmateless-chess)
